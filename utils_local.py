...
def flip_symmetric_states(df, state_col='state_type', x_col='X_aligned', y_col='Y_aligned'):
    import numpy as np
    df = df.copy()

    # Default values: no flip
    # df["flip"] = 0
    df["X_flip"] = df[x_col]
    df["Y_flip"] = df[y_col]

    # Mask for symmetric control
    mask = df[state_col] == 2
    ...
    # Rotate by -2 * angle
    df.loc[mask, "X_flip"] = np.cos(-2 * phi) * r
    df.loc[mask, "Y_flip"] = np.sin(-2 * phi) * r
    # df.loc[mask, "flip"] = 1
...
def access_utilities(utilities_name='utilities' , sub_dir=None, in_unitydataanalysis=True, unitydataanalysis_name='UnityDataAnalysis'):
    ...
    if not in_unitydataanalysis:
        ...
        sys.path.insert(1, str(Path(parent_dir) / sub_dir / unitydataanalysis_name))